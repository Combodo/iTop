...
Dict::Add('ZH CN', 'Chinese', '简体中文', [
    ...
    'Action:last_executions_tab_panel_title' => '此动作的执行 (%1$s)',
    ...
    'Class:Action/Attribute:asynchronous+' => '此动作是否可以在后台执行',
...
编辑时可点击放大镜图标获取参考示例',
    ...
    'Class:ActionNewsroom/Attribute:icon+' => '在消息空间中显示在消息旁的图标.
- 如果填写, 将使用此自定义图标
- 否则, 如果触发的对象有图标 (例如人员对象的照片), 则使用此图标
- 否则, 如果触发的对象在数据模型中定义了类图标, 则使用此图标
- 否则, 将使用应用袖珍图标',
    'Class:ActionNewsroom/Attribute:message' => '消息',
    'Class:ActionNewsroom/Attribute:message+' => '消息的正文, 格式为Markdown而非HTML. 可以包含占位符例如:
- $this->attribute_code$ 触发通知的对象的任意属性,
- $this->hyperlink()$ 触发通知的对象在后台的超链接,
- $this->hyperlink(portal)$ 触发通知的对象在门户内的超链接,
- $this->attribute_external_key->attribute$ 任何外键的属性,
- $current_contact->attribute$ 触发通知的人员的任意属性',
    ...
    'Class:ActionNewsroom/Attribute:recipients+' => '可返回联系人对象的OQL查询',
    'Class:ActionNewsroom/Attribute:test_recipient_id' => '测试收件人',
    'Class:ActionNewsroom/Attribute:test_recipient_id+' => '通知状态为测试中时接收信息的人员',
    'Class:ActionNewsroom/Attribute:title' => '标题',
    'Class:ActionNewsroom/Attribute:title+' => '消息的标题. 可以包含占位符如 $this->attribute_code$',
    'Class:ActionNewsroom/Attribute:url' => 'URL',
    'Class:ActionNewsroom/Attribute:url+' => '如果留空, 此URL将为出发该通知的对象. 您也可以指定一个URL.',
    ...
    'Class:EventNotificationEmail/Attribute:to+' => '收件人',
    'Class:EventNotificationNewsroom' => '信息已发送',
...
该图标展示在 ' . ITOP_APPLICATION_SHORT . ' 同步对象的 "锁定" 符号的提示框中',
    ...
    'Class:Trigger/Attribute:subscription_policy' => '订阅政策',
    'Class:Trigger/Attribute:subscription_policy+' => '允许用户取消订阅此触发器',
    'Class:Trigger/Attribute:subscription_policy/Value:allow_no_channel' => '允许完全取消订阅',
    'Class:Trigger/Attribute:subscription_policy/Value:force_all_channels' => '禁止取消订阅',
    'Class:Trigger/Attribute:subscription_policy/Value:force_at_least_one_channel' => '强制需要订阅一项 (消息或电子邮件)',
...
如果不写具体时间, 则默认00:00:00
    ...
    'Core:Placeholder:CannotBeResolved' => '(%1$s : 无法处理)',
