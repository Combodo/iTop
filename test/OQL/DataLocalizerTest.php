...
class DataLocalizerTest extends ItopDataTestCase
{
	...
	{
		$aData = array();
		$aData["SELECT 0"] = array("SELECT `UserRequest` FROM UserRequest AS `UserRequest` WHERE ((`UserRequest`.`service_name` NOT IN ('Sales', 'Community Support')) AND (`UserRequest`.`start_date` > '2015-06-01'))", array(), unserialize('a:1:{s:12:"grouped_by_1";s:51:"DATE_FORMAT(`UserRequest`.`start_date`, \'%Y-%m-%d\')";}'), false, array(), array(), 0, 0);
