				...
				if (!isset($aExtraParams['link_attr']))
				{
					...
					if (count($aRelations))
					{
						...
						foreach($aRelations as $sRelationCode => $aRelationInfo)
						{
							...
							{
								$aActions[$sRelationCode.'_down'] = array ('label' => $aRelationInfo['down'], 'url' => "{$sRootUrl}pages/$sUIPage?operation=navigate&relation=$sRelationCode&direction=down&class=$sClass&id=$id{$sContext}") + $aActionParams;
							}
							if (array_key_exists('up', $aRelationInfo))
							{
								$aActions[$sRelationCode.'_up'] = array ('label' => $aRelationInfo['up'], 'url' => "{$sRootUrl}pages/$sUIPage?operation=navigate&relation=$sRelationCode&direction=up&class=$sClass&id=$id{$sContext}") + $aActionParams;
							}
						}
					}
