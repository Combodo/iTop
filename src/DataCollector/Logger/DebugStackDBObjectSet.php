...
class DebugStackDBObjectSet
{
	 ...
	 * {@inheritdoc}
	 */
	public function startQuery($eventId, \DBObjectSet $DBObjectSet)
	{
		if (! $this->enabled) {
			return;
		}
		...
		if ($this->stopwatch) {
			$this->events[$eventId] = $this->stopwatch->start(get_class($DBObjectSet), 'DBObjectSet');
		}
	}
	 ...
	 * {@inheritdoc}
	 */
	public function stopQuery($eventId, \DBObjectSet $DBObjectSet)
	{
		if (! $this->enabled) {
			return;
		}
	...
	{
		return static::class.'/'.static::$iUniqueCounter++;
	}
}